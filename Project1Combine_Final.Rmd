...
#### Casualties for Incidents of Different Causes
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.align='center'}
...
These plots look at the relationship between extreme accidents and the class
...
and optimal value of lambda =-0.5 so we will do a lambda transform of the model.
...
The partial F-test from the anova function fails to reject the null hypothesis that the smaller model is no different than the larger model. We decided to select the smaller model in this case. We want to see the effect of Precip on ACCDMG so here are some calculations:
