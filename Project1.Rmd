...
### Make a barplot of the nearst station of accidents for all accidents with one or more casualties.

```{r}
# Use count to see the frequency of occurrences at each Station

y = count(totacts_wCasualties_DR, "STATION")

# Use tabel() to look at the Stations with more than nine occurrences
xstat100 <- y[y$freq > 9,]
table(xstat100)
xstat100
class(xstat100)
...
ggplot(xstat100, aes(x = STATION, y= freq)) + geom_bar(stat="identity") + theme(text = element_text(size = 5.5))
```
