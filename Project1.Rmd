...
# Use ggplot() to plot the frequency of casualties at the extreme stations.
ggplot(xstat10, aes(x = STATION, y= Count)) + geom_bar(stat="identity") + theme(text = element_text(size = 5.5))
# Use ggplot() to plot the ratio of casualties at the extreme stations.
ggplot(xstat10, aes(x = STATION, y= Ratio)) + geom_bar(stat="identity") + theme(text = element_text(size = 5.5))
##Question, how to change the scale of the y axis, the greatest has ten, but below it is three instead of being divided up properly.
```
...
xstat10$STATION
# boxplots of accident damage conditioned on accident typw
bwplot(STATION ~ Casualty, main = "Box Plots of Stations with Casualties",
xlab = "Casualties", ylab = "Stationes", data = xstat10)
# Seems like there could have been one major chicago accident?
which(totacts_wCasualties_DR$STATION == max(totacts_wCasualties_DR$STATION))
# Nope, the accident with the largest amount of casualties wasn't in Chicago. What if we create a data frame with the stations who have ten or more occurences of casualties.

hsb7 <- totacts_wCasualties_DR[totacts_wCasualties_DR$STATION %in% c("CHICAGO", "SAN ANTONIO", "BALTIMORE", "BEAUMONT", "FORT WORTH", " FRESNO", "GARY", "HOUSTON", "KANSAS CITY", "MADERA", "MEMPHIS", "NEW YORK NEW YORK", "PHILADELPHIA"), ]
bwplot(STATION ~ Casualty, main = "Box Plots of Stations with Casualties",
xlab = "Casualties", ylab = "Stations", data = hsb7)

```
```{r}
# Create the Chicago variable to look explicitly at casualties in Chicagor
totacts_wCasualties_DR$Chicago <- (totacts_wCasualties_DR$STATION == "CHICAGO")
totacts_wCasualties_DR$NChicago <- (totacts_wCasualties_DR$STATION != "CHICAGO")
# Get interaction plot for Derail and TRNSPD
# Define cut point for high and low speed
Casual <- cut(totacts_wCasualties_DR$Casualty, c(1, mean(totacts_wCasualties_DR$Casualty), max(totacts_wCasualties_DR$Casualty)),
include.lowest = T, labels = c("low casualties", "high casualties"))
# Plot interaction between Chicago and Casualties
interaction.plot(totacts_wCasualties_DR$Chicago, totacts_wCasualties_DR$NChicago, totacts_wCasualties_DR$Casualty)
